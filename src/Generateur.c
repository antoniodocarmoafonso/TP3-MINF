...
void GENSIG_UpdateSignal(S_ParamGen *pParam)
{
    ...
    Offset = pParam->Offset * -1;
    //Conversion de l'amplitude saisie par l'utilisateur en mV
    amplitude_en_mV = (pParam ->Amplitude / 100) * 2 ;
    ...
    for(indexEchantillons = 0; indexEchantillons < MAX_ECH; indexEchantillons++)
    {
        ...
        switch (pParam->Forme) 
        {
            case SignalSinus:
                ...
                formeSignal[indexEchantillons] = (tbSinus[indexEchantillons] - 50) * amplitude_en_mV + MOITIE_STEP_AMPLITUDE + Offset;
                break;

            case SignalCarre:
                if (indexEchantillons < MAX_ECH * 0.5)
                {
                    //Calcul des 50 échantillons du signal carré négatif
                    formeSignal[indexEchantillons] = MOITIE_STEP_AMPLITUDE - ((amplitude_en_mV * MAX_ECH)* 0.5 - Offset);
                }
                else
                {
            ...
            case SignalDentDeScie:
                ...
                formeSignal[indexEchantillons] = Offset + (indexEchantillons - 50) * amplitude_en_mV + MOITIE_STEP_AMPLITUDE;
                break;

            case SignalTriangle:
                if((MAX_ECH * 0.5) < indexEchantillons)
                {
                    //Calcul des 50 échantillons du signal triangle positif
                    formeSignal[indexEchantillons] = amplitude_en_mV * (100 - 2 * (indexEchantillons - 25)) + Offset + MOITIE_STEP_AMPLITUDE; 
                }
                else
                {
                    //Calcul des 50 échantillons du signal triangle négatif
                    formeSignal[indexEchantillons] = amplitude_en_mV * (2 * (indexEchantillons - 25)) + Offset + MOITIE_STEP_AMPLITUDE;
                }
                break;
